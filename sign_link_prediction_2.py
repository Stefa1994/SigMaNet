...
def parameter_parser():
    ...
    parser.add_argument('--split_prob', type=lambda s: [float(item) for item in s.split(',')], default="0.01,0.20", help='random drop for testing/validation/training edges (for 3-class classification only)')
    parser.add_argument('--task', type=str, default='sign', help='Task')
...
def best_run(test_loss, best_test_err, log_path, early_stopping):
                ####################
            # Save weights
            ####################
...
for split in list(link_data.keys()):
    ...
    #for q in [11, 22, 33, 44, 55]:
    #    curr_graph = SignedData(edge_index=edge_index, edge_weight=edge_weight)
    #    q = q/55 * 0.5/(curr_graph.A - curr_graph.A.transpose()).max()
    #    print(q)
    #exit()
    ...
    if args.method == 'SGCN':
        ...
        model = SigMaNet_link_prediction_one_laplacian(K=1, num_features=num_input_feat, hidden=args.hidden, label_dim=2,
        ...
        edge_index, norm_real, norm_imag_i, norm_imag_j, norm_imag_k  = quaternion_laplacian.process_quaternion_laplacian(edge_index=edge_index, x_real=X_real, edge_weight=edge_weight, \
         normalization = 'sym', return_lambda_max = False)
        model = QuaNet_link_prediction_one_laplacian(K=args.K, num_features=num_input_feat, hidden=args.hidden, label_dim=2,
    ...
    if args.method == 'MSGNN':
        ...
        test_acc, f1, f1_macro, f1_micro, auc = test_QuaterGCN(X_real, X_img_i, X_img_j, X_img_k, y_test, query_test_edges)
        #print(f'Split: {split:02d}, Test_Acc: {test_acc:.4f}, F1: {f1:.4f}, F1 macro: {f1_macro:.4f}, \
        #    F1 micro: {f1_micro:.4f}, AUC: {auc:.4f}')
    else:
        for epoch in range(args.epochs):
