...
class SymModel(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_in, in_w, edge_out, out_w, edge_weight = None):
        x = self.lin1(x)
        x1 = self.gconv(x, edge_index, edge_weight)
...
class Sym_Link(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_in, in_w, edge_out, out_w, index, edge_weight = None):
        x = self.lin1(x)
        x1 = self.gconv(x, edge_index, edge_weight)
        x2 = self.gconv(x, edge_in, in_w)
        x3 = self.gconv(x, edge_out, out_w)
