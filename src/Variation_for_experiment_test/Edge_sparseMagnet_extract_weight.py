...
def main(args):
    ...
    for i in range(10):
        ...
        L = hermitian_decomp_sparse(f_node, e_node, size, args.q, norm=args.not_norm, laplacian=True,  max_eigen = 2.0, gcn_appr = False, edge_weight = datasets[i]['weights'])       
        ...
        # SE voglio solo peso sul laplaciano
        ...
        print('accuratezza ', test_err)
        rest = []
        for name, param in model.named_parameters():
            print(name)
            if param.requires_grad:
                if len(param.data) == 2 and name in ['Chebs.0.weight', 'Chebs.2.weight']:
                    print(name, param.data[0].size(), torch.norm(param.data[0]), torch.sum(torch.abs(param.data[0])))
                    print(name, param.data[1].size(), torch.norm(param.data[1]), torch.sum(torch.abs(param.data[1])))
                    rest.append([torch.norm(param.data[0]).cpu().detach().numpy(), torch.sum(torch.abs(param.data[0])).cpu().detach().numpy(),
                    torch.norm(param.data[1]).cpu().detach().numpy(), torch.sum(torch.abs(param.data[1])).cpu().detach().numpy()])
        casa = [item for sublist in rest for item in sublist]
        results[i] = casa
