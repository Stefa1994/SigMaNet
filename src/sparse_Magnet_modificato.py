...
from utils.edge_data import load_signed_real_data_no_negative
...
def main(args):
    ...
    for split in range(splits):
        ...
        count = np.sum(val_index)
        acc_train =  acc(pred_label, label, val_index) #(1.0*((pred_label[:,val_index] == label[:,val_index])).sum().detach().item())/count

        count = np.sum(test_index)
        acc_test =  acc(pred_label, label, test_index) #(1.0*((pred_label[:,test_index] == label[:,test_index])).sum().detach().item())/count
